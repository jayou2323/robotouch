...
import StatsPage from './stats_page'; // 새로 추가된 스탯창 페이지 컴포넌트
...
// 컴포넌트 함수 Chatbot_main 내부에서 상태 초기화
function Chatbot_main() {
  ...
  const [isStatsOpen, setIsStatsOpen] = useState(false); // 스탯창 페이지 상태
  ...
  const handleSidebarClick = (item) => {
    console.log(`Clicked on: ${item}`); // 디버깅을 위한 로그
    if (item === '스탯창 보기') {
      setIsStatsOpen(true);
    } else {
    ...
    <AppContainer>
      ...
      <Sidebar>
        ...
        {sidebarItems.map((text, index) => (
          <SidebarItem key={index} onClick={() => handleSidebarClick(text)}>
            {text}
          </SidebarItem>
        ))}
        <SidebarItem key="stats" onClick={() => handleSidebarClick('스탯창 보기')}>
          스탯창 보기
        </SidebarItem>
      </Sidebar>
      ...
      <Main>
        {isStatsOpen ? (
          ...
          <>
            ...
            <ChatboxArea
              onMouseEnter={() => setIsHoverChatbox(true)}
              onMouseLeave={() => setIsHoverChatbox(false)}
            >
              <Chatbox
                setInput={setInput}
                handleKeyPress={handleKeyPress}
                setIsHoverChatbox={setIsHoverChatbox}
              />
            </ChatboxArea>
          </>
        )}
